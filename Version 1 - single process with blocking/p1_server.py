...
""" p1_server.py: Example pipeline server (P1).
...
def pipeline_p1(loop):
    ...
    async def mh_s1(msg):
        ...
        data = sync_null_transform(jresponse, test_pauses[0])  # Stage processing
    ...
    async def mh_s2(msg):
        ...
        data = sync_null_transform(jrequest, test_pauses[1])  # Stage processing
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='../logs/p1_pipeline.log', filemode='w', format='%(asctime)s, %(message)s',
                        level=logging.DEBUG)
