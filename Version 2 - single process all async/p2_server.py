...
""" p2_server.py: Example pipeline server (p2).
...
def pipeline_p2(loop):
    ...
    async def mh_s1(msg):
        ...
        log('info', 'p2', 's0', 'S1 initiated', correlation_id)
        ...
        await nc.publish("p2.s1", json.dumps(data).encode('utf-8'))
        log('info', 'p2', 's1', 'S1 completed', correlation_id)
    ...
    async def mh_s2(msg):
        ...
        log('info', 'p2', 's2', 'S2 completed', data['correlation_id'])
        await nc.publish("p2.s2", msg.data)
    ...
    async def mh_s3(msg):
        ...
        log('info', 'p2', 's3', 'S3 completed', data['correlation_id'])
        await nc.publish("p2.s3", msg.data)
    ...
    async def mh_s4(msg):
        ...
        log('info', 'p2', 's4', 's4 completed', data['correlation_id'])

    ######################
    # Pipeline Creation
    ######################
    yield from nc.subscribe("p2.s0", cb=mh_s1)
    yield from nc.subscribe("p2.s1", cb=mh_s2)
    yield from nc.subscribe("p2.s2", cb=mh_s3)
    yield from nc.subscribe("p2.s3", cb=mh_s4)


if __name__ == '__main__':
    import logging

    logging.basicConfig(filename='../logs/p2_pipeline.log', filemode='w', format='%(asctime)s, %(message)s',
                        level=logging.DEBUG)
    ...
    event_loop.run_until_complete(pipeline_p2(event_loop))
